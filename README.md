...
Se ha implementado un algoritmo de control continuo de tres posiciones con histéresis y zona muerta, es decir, con acciones proporcionales continuas al error de la medición con respecto al valor deseado. Por ejemplo, si el error es solo de 1 grado se abre solo el 4.6% de una válvula de control continua y así sucesivamente se incrementa un 4.6% de apertura de la válvula continua por cada grado de diferencia. En este caso sería un control tipo P con ganancia K = 0.046 para cada acción de control en rampa, enfriar y calentar. Se selecciona este algoritmo porque permite acciones de control en ambos sentidos (calentar y enfriar) y la conmutación se hace en un rango de  +/-3 grados alrededor del valor deseado 25 ºC. La histéresis la conseguimos utilizando variables tipo "int" que hacen que no actúe el sistema con valores decimales. 
